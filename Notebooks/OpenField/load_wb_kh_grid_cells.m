...
% locally mount data. 
% in terminal:
% sshfs alexg8@login.sherlock.stanford.edu:/oak/stanford/groups/giocomo/export/data/Users/WButler /home/alexgonzalez/oak_wb/
%%
data_info_dir = '~/Documents/data/butler_hardcastle/';
file_name = 'wb_kh_data.mat';

% loads two variables:
% 1) cell_info_trained
% 2) grid_cells_freeForaging
load(strcat(data_info_dir,file_name));

data_path = '/home/alexgonzalez/oak_wb/D254 Neuralynx';
grid_cells_file_ids = (grid_cells_freeForaging);

n_units = length(grid_cells_file_ids);
...
for ii = 1: n_units
    ...
    temp = strsplit(grid_cells_file_ids{ii},'_');
    subjects(ii) = temp(1);
    dates(ii) = temp(2);
    times(ii) = temp(3);
    channels(ii) = temp(4);
...
unit_info_table = table(units, sessions_id, session_units_id, subjects, ...
    dates, times, channels, sessions, grid_cells_file_ids);
writetable(unit_info_table,fullfile(data_info_dir, 'grid_cell_info_table.csv'))
...
save_file_name = fullfile(data_info_dir,'grid_cell_data.h5');
...
for session_id = 1:n_sessions
    ...
    for unit = unit_ids
        ...
        spk_fn = strcat(grid_cells_file_ids{unit},'.mat');
    ...
    for unit = unit_ids
        h5writeatt(save_file_name, strcat('/session_',string(session_id),'/neural_data'),...
            strcat('orig_file_unit_',string(unit_counter)), grid_cells_file_ids{unit});
        unit_counter = unit_counter + 1;
    end
end
